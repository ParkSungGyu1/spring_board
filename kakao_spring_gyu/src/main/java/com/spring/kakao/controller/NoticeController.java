...
public class NoticeController {
	...
	public ModelAndView noticeIndex(@RequestParam String pageNumber, HttpServletRequest request) {
		...
		mav.addObject("noticeList", noticeService.getNoticeList(noticeService.parsePageNUmber(pageNumber)));
	...
	public byte[] noticeDtlFileDownload(HttpServletResponse response,
		...
		String encodingOrginFileName = null;
		try {
			encodingOrginFileName = new String(originFileName.getBytes("UTF-8"), "ISO-8859-1");
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		response.setHeader("Content-Disposition", "attachment;filename=\""+encodingOrginFileName+"\"");
